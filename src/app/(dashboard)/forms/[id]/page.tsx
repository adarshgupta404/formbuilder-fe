...
async function SubmissionsTable({ formb }: { formb: FormBuilder }) {
  const form = await GetFormWithSubmissions(formb._id);

  if (!form) {
    console.log("No Forms Found!");
  }
  const formElements = JSON.parse(formb.content) as FormElementInstance[];
  const columns: {
    id: string;
    label: string;
    required: boolean;
